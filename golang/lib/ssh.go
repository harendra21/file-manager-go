...
func RunCommand(command string) (string, error) {
	...
	ssh, err := NewSshClient("ubuntu", "140.238.250.140", 22, "/Users/harendra/go/src/projects/Angular/sitepro/golang/lib/ssh-key-2023-01-06.key", "")

	if err != nil {
		log.Printf("SSH init error %v", err)
		return "", err
	} else {
		output, err := ssh.RunCommand(command)
